-- WOOX - Reconstrucción de get_compiled_prompt con Soporte Global de Placeholders
-- Esta versión asegura que {{catalogContext}} y {{merchantName}} se reemplacen en 
-- las instrucciones del Agente, del Comercio y de las Skills.
...
BEGIN
    ...
    -- 5. ENSAMBLAR PROMPT (Concatenando todas las fuentes)
    v_final_prompt := '### TU ROL: Asistente Concierge de ' || v_merchant.name || E'.\n\n' ||
                      '### IDENTIDAD BASE:\n- Personalidad: ' || v_personality || E'.\n- Saludo: ' || v_welcome || E'\n\n';

    -- Inyectar instrucciones del Agente Maestro
    ...
    IF v_knowledge_agent IS NOT NULL THEN
        v_final_prompt := v_final_prompt || '### CONOCIMIENTO MAESTRO:\n' || v_knowledge_agent || E'\n\n';
    END IF;
    IF v_knowledge_merchant IS NOT NULL THEN
        v_final_prompt := v_final_prompt || '### CONOCIMIENTO ESPECÍFICO DEL LOCAL:\n' || v_knowledge_merchant || E'\n\n';
    END IF;

    -- Inyectar Restricciones e Instrucciones del Comercio
    ...
    -- Inyectar el Catálogo al final como Sección Oficial
