-- WOOX - Re-corrección de get_compiled_prompt (Subconsulta para Agregación)
-- Esta versión usa una subconsulta para evitar errores de agregación/ordenamiento.
...
BEGIN
    ...
    -- 3. Generar el BLOQUE DE CATÁLOGO (Uso de Subconsulta para evitar errores de GROUP BY)
    IF COALESCE(v_merchant.ai_use_catalog, true) = true AND v_merchant.industry_type != 'support' THEN
        SELECT string_agg(
            '➔ [' || COALESCE(cat_name, 'Otros') || '] ' || p_name || ' | Precio: $' || p_price || ' | [DISPONIBLE]',
            E'\n' ORDER BY cat_name, p_name
        ) INTO v_catalog
        FROM (
            SELECT p.name as p_name, p.price as p_price, c.name as cat_name
            FROM products p
